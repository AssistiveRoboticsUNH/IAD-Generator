        ...
        for i, m in enumerate(confidences[0]):
            ...
            for j, p in enumerate(m):
                ...
                if j in range(5):
                    ...
                    print(label, len(confidence),  i, len(confidence_discount_layer))
...
def get_data_train(iad_list):
    ...
    batch_indexs = np.random.randint(0, len(iad_list["data"][0]), size=BATCH_SIZE)
...
def train_model(model, train, test, num_classes):
    ...
    with tf.Session() as sess:
        ...
        num_iter = EPOCHS * len(train["data"][0]) / BATCH_SIZE
...
def locate_iads(files):
    ...
    return {"data":data_arr, "label":labels_arr, "length":length_arr}
...
def main():
    ...
    eval_files = [x for x in os.listdir(args.test) if x.find("test") >= 0 and x.find("pruned") < 0]
